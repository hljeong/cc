...
check 'assign and read' '5' '{int a=5;a;}'
check 'assign two vars' '10' '{int a=5;int b=10;b;}'
check 'chain assign' '10' '{int a; int b; a=b=10;a;}'
check 'long variable name' '10' '{int abc; int bcd; abc=bcd=10;abc;}'
...
check 'block in block' '2' '{1; {int x = 5; return 2;} 3;}'
check 'empty block' '0' '{{} 0;}'
check 'empty statement' '3' '{ int _; ;; ;_; return 3 ;_; ;; }'
check 'if' '3' '{ if (2 < 3) return 3; return 2; }'
check 'if else' '2' '{ if (1 + 1 > 3) return 3; else { return 2; } }'
check 'for' '45' '{ int i = 0; int j = 0; for (i = 0; i < 10; i = i + 1) j = i + j; return j; }'
check 'forever' '1' '{ for (;;) return 1; return 2; }'
check 'while' '15' '{ int i = 0; while (i < 15) i = i + 1; return i; }'
check 'addr-deref' '3' '{ int x = 3; return *&x; }'
check 'pointer' '2' '{ int x = 3; int *y = &x; *y = 2; return x; }'
check 'double pointer' '154' '{ int x = 3; int y = 5; int *z = &y; int **w = &z; **w = 7; *w = &x; **w = 11; return x * (y + y); }'
check 'pointer plus' '4' '{ int x = 3; int y = 5; *(&y + 1) = 4; return x; }'
check 'pointer sub' '4' '{ int x = 3; int y = 5; *(&x - 1) = 4; return y; }'
check 'pointer dist' '1' '{ int x = 3; int y = 5; return &x - &y; }'
check 'pointer arithmetic' '6' '{ int x = 3; int y = 5; *(&y - 20 + 3 * 7) = y; return x + (&x - &y); }'
check 'pointer arithmetic' '6' '{ int x = 3; int y = 5; *(&y - 20 + 3 * 7) = y; return x + (&x - &y); }'
check 'multiple declarations' '14' '{ int x = 3, y = 5; return x * y - 1; }'
check 'refer to declared vars in same stmt' '3' '{ int x = 3, *y = &x, z = *y; z; }'
